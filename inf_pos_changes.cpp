    ...
    for (i = 0; i < input.length(); i++)
    {
        ...
        if (input[i] != ' ')
        {
            ...
            else if (s.empty() || precedence(input[i]) > precedence(s.top()))
            {
                ...
                // decimal_status = 0;
                cout << "add " << add_sub_status << " MUL " << mul_div_bracket_status << endl;
                if (num_status == 1)
                {
                    temp_string = to_string(sign * temp);
                    postfix.push(temp_string);
...
/*int main()
{
    // string input = "{(5.89*23+1)(2)}/(-5+3*7-6)";
    string input = "5";
    queue<string> postfix = infix_to_postfix(input);
    queue<string> postfix_temp = postfix; // infix_to_postfix(input);
    while (!postfix_temp.empty())
    {
        cout << postfix_temp.front() << endl;
        postfix_temp.pop();
    }
    double t = ans(postfix);
    cout << "ans" << t << endl;
